  ...
  try {
    ...
    const message = {
      notification: {
        ...
        body: `${alert.alertType}: $${currentPrice.toFixed(2)} ${
          alert.alertType === 'STOP' ? '⚠️' : '✅'
        }`
      },
      data: {
        type: 'price_alert',
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      for (const userDoc of usersSnapshot.docs) {
        ...
        const watchlistNotifications = userData.notificationPreferences?.watchlistUpdates;
        const highConfidenceNotifications = userData.notificationPreferences?.highConfidence;
