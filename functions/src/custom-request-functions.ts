 ...
 */
export const submitCustomReportRequest = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  if (!ticker || !assetType) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'ticker and assetType are required'
    );
  }
  ...
  try {
    ...
    if (!existingRequest.empty) {
      throw new functions.https.HttpsError(
        'already-exists',
        'You already have a pending request for this ticker'
      );
    }
 ...
 */
export const getUserCustomRequests = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
